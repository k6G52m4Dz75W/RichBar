...
- **运行时绘制、主题自适应图标**：内嵌 Remix Icon 字体子集覆盖全部 20 个
  ...
  自动取反（浅底深字 / 深底浅字）；深色栏时控件切换到系统深色工具栏主题
  （`DarkMode_Explorer`），悬停与按下填充随之使用深色配色；内嵌的下拉
  三角随字形配色自动取反。主题/配置
  变化自动重绘。
...
- **下拉标识内嵌于图标（Word 式分体布局）**：工具栏维护两张图像列表——
  一张纯单元格宽度的普通列表供所有按钮使用；另一张"字形单元 + 右侧
  箭头窄条"的加宽列表仅供三个下拉按钮（标题、字体、窗体）使用，它们
  的按钮加宽一个窄条，窄条内居中内嵌一个小实心三角（Remix
  `arrow-down-s-fill` 字形），与其余图形使用同一前景色绘制。字形本身
  一像素不动，普通按钮的宽度和观感与加窄条之前完全一致。这三个按钮
  保留 `BTNS_DROPDOWN` 下拉语义，但工具栏不设 `TBSTYLE_EX_DRAWDDARROWS`
  扩展样式，控件因此不绘制任何箭头，不会出现异色图形；整颗按钮点击
  发送 `TBN_DROPDOWN` 打开菜单，悬停同样会自动展开。
- **旧版 HTML 彩色 BMP 不再被加载**（资源仍保留在 DLL 中，插件列表中的
  插件入口图标仍使用自己的位图）。深色栏时控件切换到深色工具栏主题，
  悬停与按下填充均为深色系，浅色字形全程可读。
