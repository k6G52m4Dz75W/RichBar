...
- **运行时绘制、主题自适应图标**：内嵌 Remix Icon 字体子集覆盖全部 20 个
  ...
  （`DarkMode_Explorer`），悬停填充与下拉箭头随之使用深色配色。主题/配置
  变化自动重绘。
...
- **下拉标识内嵌于图标**：栏内三个下拉按钮（标题、字体、窗体）的图标
  位图中内嵌一个小实心三角，与其余图形使用同一前景色绘制。工具栏控件
  不再自绘任何箭头，因此不会出现异色图形。这三个按钮为普通按钮；点击
  产生普通 `WM_COMMAND` 并打开菜单，悬停同样会自动展开。
