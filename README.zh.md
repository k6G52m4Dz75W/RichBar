...
- **下拉标识内嵌于图标（Word 式分体布局）**：下拉按钮的位图由"完整字形
  单元 + 右侧专用箭头窄条"组成，三个下拉按钮（标题、字体、窗体）的
  窄条内居中内嵌一个小实心三角（Remix `arrow-down-s-fill` 字形），与
  其余图形使用同一前景色绘制，字形本身一像素不动；其余按钮的字形在
  整幅图像宽度上居中，与旧版纯单元格图像的按钮位置完全一致。这三个
  按钮保留 `BTNS_DROPDOWN` 下拉语义，但工具栏不设 `TBSTYLE_EX_DRAWDDARROWS`
