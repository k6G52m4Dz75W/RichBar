...
- **图标颜色自定**：插件属性对话框新增"图标颜色"行——**自动**（默认，
  ...
  **工具栏上的"图标颜色"按钮**（弹出菜单：自动 / 自定义颜色…）等效，
  旧版保存的按钮布局会自动补上这个新按钮。
- **设计视图与预览按钮**：*Design View* 切换 EmEditor 的 Markdown 设计
  视图（`EEID_MARKDOWN_VIEW`），*Preview* 切换 HTML/Markdown 网页预览
  （`EEID_VIEW_WEB`）——均为 v24.4 SDK 的内置命令，经 `WM_COMMAND` 执行。
  同样会自动补进旧版保存的布局，配专用 `markdown-line` / `eye-line` 字形。
