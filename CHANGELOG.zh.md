...
- 版本号直接升至 0.10.0（而非 0.9.1）——全面更名与设置存储的独立，
  其改动规模超出了一个补丁版本的范畴。
...
- **在非英文（如中文）界面语言的系统上，插件列表中的名称显示为空白**。
  EmEditor 按"界面语言的资源标签精确匹配"规则从 `mui\<LCID>` 卫星 DLL 中解析
  插件字符串，而此前 `mui\2052` 里放的只是英文标签（0x409）二进制的原样拷贝。
  现在卫星资源脚本通过 `LOC_LANG_2052` 预处理宏选择资源语言，
  `tools/build-loc-2052.ps1` 负责构建打上简体中文（0x804）标签的 `mui\2052` 版本。
