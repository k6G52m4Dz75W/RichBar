...
- **插件版本显示为陈旧的字符串 "14"** 而不是 0.10.1：EmEditor 的版本列来自
  插件的 `EP_GET_VERSION` 字符串（卫星中的 `IDS_VERSION`），而不是文件本身的
  VERSIONINFO 资源。该字符串自 fork 原版 HTMLBar 起就一直未被改动，现更新为 0.10.1。
  在上述卫星加载修复之前，版本查询失败，EmEditor 会静默退回读取 VERSIONINFO
  资源，因此一直掩盖了这个问题。
